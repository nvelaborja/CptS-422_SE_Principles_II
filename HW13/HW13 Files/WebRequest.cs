...
namespace CS422
{
	public class WebRequest
	{
		...
		public WebRequest (NetworkStream netStream, byte[] buffer)									// buffer includes data we already got from the netStream in the Web Server
		{
			...
			string previousData = System.Text.Encoding.ASCII.GetString (buffer);					// Convert buffer to a string
			...
			foreach (string header in headerStrings) {
				if (header == "")
					continue;
				string[] headerPieces = header.ToLower().Split(":".ToCharArray(), 2, StringSplitOptions.None);	// header string should only have two pieces, one colon
		...
		private long GetLength() 			// Check to see if there is a Content-Length header
		{
			if (headers.ContainsKey ("Content-Length".ToLower ())) 
				return Convert.ToInt64(headers ["Content-Length".ToLower ()]);
		...
		{
			// Handle all % encodings we might see in request uris, add more as you see them.
			HTTPUri = HTTPUri.Replace ("%20", " ");		
		}
		...
		//public void WriteToNetStream(byte[] buffer, int offset, int size)
		//{
