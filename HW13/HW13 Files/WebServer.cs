...
namespace CS422
{
	public class WebServer
	{
		...
		private static WebRequest BuildRequest(TcpClient client)
		{
			...
			Stopwatch requestTimer = new Stopwatch();							// Timer for entire request validation process
			...
			requestTimer.Start ();												// Start the request timer

			while (!foundEnd) { 
				if (requestTimer.ElapsedMilliseconds > 10000){					// Check request timer, make sure we haven't gone beyond 10 seconds
					netStream.Dispose ();
					return null;
				}
				...
				try {
					i = netStream.Read (buffer, bytesRead, buffer.Length - bytesRead);	// Receive request data from client
				} catch (TimeoutException ex) {
					netStream.Dispose ();
					return null;
				} catch (System.IO.IOException ex) {
					// Do nothing here, IOException means natural end of netStream
				}

				bytesRead += i;
				data = System.Text.Encoding.ASCII.GetString (buffer, 0, bytesRead);		// Convert what we got to a string

				if (!ThresholdTest (data)) {											// Make sure the data we have so far passes our threshold requirements
					netStream.Dispose ();
					return null;
				}
		...
		private static bool ValidateRequest(string data)
		{
			...
			if (!(data.StartsWith ("GET ") || (data.StartsWith("PUT ")))) {	 // Reject requests other than GET and PUT
				return false;
			}
		...
		private static bool ThresholdTest(string data)
		{
			int requestInfoThreshold = 2 * 1024;								// Set threshold for request info line to 2KB
			int headerTheshold = 100 * 1024;									// Set threshold for entire header to 100KB

			if (data.Contains ("\r\n")) {										// If we have already seen a CRLF, make sure it was before the threshold
				if (data.IndexOf ("\r\n") > requestInfoThreshold)
					return false;
			} 
			else {																// Otherwise, make sure our length is less than the info threshold
				if (data.Length > requestInfoThreshold)
					return false;
			}

			if (data.Contains ("\r\n\r\n")) {									// If we have already seen a double CRLF, make sure it was before the threshold
				if (data.IndexOf ("\r\n\r\n") > headerTheshold)
					return false;
			} 
			else {																// Otherwise, make sure our length is less than header threshold
				if (data.Length > headerTheshold)
					return false;
			}
